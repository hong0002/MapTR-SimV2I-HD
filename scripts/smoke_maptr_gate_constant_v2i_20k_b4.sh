...
CONFIG="integrations/maptr/configs/simv2i_maptr_gate_constant_v2i_r18_20k_b4_smoke.py"
WORK_DIR="${MAPTR_TRAIN_WORK_DIR:-outputs/maptr/gate_constant_v2i_r18_20k_b4_smoke}"
LOG_DIR="${WORK_DIR}/logs"
GPUS="${GPUS:-4}"
PORT="${PORT:-28830}"
EXPECTED_GATE_MODE="constant"
EXPECTED_CONSTANT_GATE_VALUE="0.2"
mkdir -p "${LOG_DIR}"
...
echo "Running Constant Gate V2I 20k batch4 config/dataloader/model precheck..."
SMOKE_GPUS="${GPUS}" \
SMOKE_CONFIG="${CONFIG}" \
SMOKE_WORK_DIR="${WORK_DIR}" \
EXPECTED_GATE_MODE="${EXPECTED_GATE_MODE}" \
EXPECTED_CONSTANT_GATE_VALUE="${EXPECTED_CONSTANT_GATE_VALUE}" \
"${PYTHON_BIN}" - <<'PY'
import json
import os
import pickle
...
expected_constant = float(os.environ['EXPECTED_CONSTANT_GATE_VALUE'])
...
assert abs(model.pose_gate.constant_gate_value - expected_constant) < 1e-8
pose_gate_param_count = sum(p.numel() for p in model.pose_gate.parameters())
assert pose_gate_param_count == 0, pose_gate_param_count
...
assert gate_stats['learnable_gate'] is False, gate_stats
...
assert abs(gate_stats['gate_mean'] - expected_constant) < 1e-6, gate_stats
